...
export default function ParticipantList({ registrations }) {
  const visible = registrations.filter(r => r.payment_status !== 'rejected')
  const confirmed = visible.filter(r => r.payment_status === 'confirmed')
  const pending = visible.filter(r => r.payment_status === 'pending')

  // Gender stats (confirmed only)
